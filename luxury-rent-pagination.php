...
if (isset($_GET['filter-search']) && $_GET['filter-search'] == 'full-search') {
    $sql  = "SELECT * FROM property WHERE status='RT' AND propertyType='CO' AND ";
    ...
    $rent = $conn->query("SELECT * FROM property WHERE status='RT' AND propertyType='CO' ORDER BY RAND() LIMIT $start_from, $limit"); 
}
